...
public class GetAllSettingsParser {
    ...
    public static GetAllSettingsModel parse(byte[] data) {
        ...
        try {
            ...
            log.info("Чтение Life_Time_Week...");//31...32  (104 Life_Time_Week)
            value = (short) (buffer.get() & 0xFF);
            log.info("Life_Time_Week: " + value);
            settings.setLifeTimeWeek(value);
            log.warn("Осталось байт: " + buffer.remaining());
            ...
            log.warn("SNSRef_ADC skipped (skipped)" + value);
            log.warn("Осталось байт: " + buffer.remaining());
            ...
            log.info("Чтение ScreenPosition...");//65...66 (crPos)
            value = (short) (buffer.get() & 0xFF);
