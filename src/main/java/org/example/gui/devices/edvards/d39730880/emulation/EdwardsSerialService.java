...
public class EdwardsSerialService {
    ...
    private final SerialPortDataListener listener = new SerialPortDataListener() {
        ...
        @Override public void serialEvent(SerialPortEvent event) {
            if (event.getEventType() == SerialPort.LISTENING_EVENT_DATA_AVAILABLE) readAvailableData();
        }
    };

    public EdwardsSerialService(Function<String, String> commandHandler) {
    ...
    private void processBuffer() {
        byte[] data;
        synchronized (buffer) { data = buffer.toByteArray(); }

        int crIndex = -1;
        for (int i = 0; i < data.length; i++) if (data[i] == '\r') { crIndex = i; break; }

        if (crIndex >= 0) {
            String cmd = new String(data, 0, crIndex, StandardCharsets.US_ASCII).trim();
            synchronized (buffer) {
                buffer.reset();
                if (data.length > crIndex + 1) buffer.write(data, crIndex + 1, data.length - crIndex - 1);
            }
            if (!cmd.isEmpty()) {
                log.info("Edwards RX: " + cmd);
                String response = commandHandler.apply(cmd);
                if (response != null && !response.isEmpty()) sendResponse(response);
            }
        }
    }
    ...
    private void sendResponse(String response) {
        if (port == null || !port.isOpen()) return;
        if (!response.endsWith("\r")) response += "\r";
        byte[] bytes = response.getBytes(StandardCharsets.US_ASCII);
        port.writeBytes(bytes, bytes.length);
        log.info("Edwards TX: " + response.trim());
    }
    ...
    public synchronized boolean openPort(String portName, int baudRate) {
        ...
        if (port.openPort()) {
            ...
            log.info("Edwards порт открыт: " + portName);
            return true;
        }
        return false;
    }
