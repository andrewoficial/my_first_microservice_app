...
/**
 * Автономный COM-транспорт для ARD_FEE_BRD_METER (ASCII, CR).
 * Стандарт платы: 57600 8N1.
 * <p>
 * Multiline mode (GCOEF/REBOOT): lines accumulate until idle gap, delivered as one block with {@code \n}.
 */
@Slf4j
public final class FeeBoardCommunicationService {
    ...
    public boolean openPort(String portName, int baudRate, int parity) {
        ...
        if (!port.openPort()) {
            log.error("FeeBoard: не удалось открыть порт {}", portName);
            return false;
        }
        ...
        if (activeBaud != baud) {
            log.warn("FeeBoard: запрошен baud {}, фактически {}", baud, activeBaud);
        }

        running = true;
        startReader();
        log.info("FeeBoard: порт открыт {} @ {} 8{}1 (requested {})",
     ...
     * (for GCOEF / REBOOT multi-line dumps).
     */
    ...
    public void sendCommand(String cmd) {
        ...
        log.info("FeeBoard TX ({}): {}", written, cmd.trim());
    }
        ...
        readerThread = new Thread(() -> {
            ...
            while (running && port != null && port.isOpen()) {
                try {
                    ...
                    if (running) {
                        log.error("FeeBoard read error", e);
                    }
                }
            }
        }, "FeeBoard-Reader");
    ...
    private void flushBuffer() {
        ...
        String text;
        if (multilineMode) {
            // keep line structure for GCOEF / REBOOT
        ...
        log.info("FeeBoard RX: {}", text.length() > 120 ? text.substring(0, 120) + "…" : text.replace('\n', '|'));
        for (Consumer<String> l : responseListeners) {
            try {
                l.accept(text);
            } catch (Exception ex) {
                log.warn("FeeBoard listener error: {}", ex.getMessage());
            }
        }
    }
}
