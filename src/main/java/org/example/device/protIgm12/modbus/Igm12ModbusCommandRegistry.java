...
public class Igm12ModbusCommandRegistry extends DeviceCommandRegistry {
    ...
    private SingleCommand createGetConcCommand() {
        ...
        //строка 03 00 06 00 01 - номер команды, номер регистра и читаемая длинна
    ...
    private AnswerValues parseMeasurementResponse(byte[] response) {
        if (response.length < 7 || response[0] != (byte) slaveAddress || response[1] != 0x03 || response[2] != 0x02) {
            log.warn("IGM10: Invalid measurement response");
            return null;
        }
        if (!validateCRC(response)) {
            log.warn("IGM10: CRC error in measurement response");
        }
        ...
        answerValues.addValue(concentration, "Unit ");
    ...
    private AnswerValues parseVersionResponse(byte[] response) {
        if (response.length < 7 || response[0] != (byte) slaveAddress || response[1] != 0x03) {
            log.warn("IGM10: Invalid version response");
            return null;
        }
        if (!validateCRC(response)) {
            log.warn("IGM10: CRC error in version response");
        }
    ...
    private AnswerValues parseSerialResponse(byte[] response) {
        if (response.length < 9 || response[0] != (byte) slaveAddress || response[1] != 0x03 || response[2] != 0x04) {
            log.warn("IGM10: Invalid serial response");
            return null;
        }
        if (!validateCRC(response)) {
            log.warn("IGM10: CRC error in serial response");
        }
    ...
    private AnswerValues parseGasPropertyResponse(byte[] response) {
        if (response.length < 7 || response[0] != (byte) slaveAddress || response[1] != 0x03) {
            log.warn("IGM10: Invalid gas property response");
            return null;
        }
        if (!validateCRC(response)) {
            log.warn("IGM10: CRC error in gas property response");
        }
        ...
        String unitStr = getIgm10GasName(gasCode) + " %НКПР"; // Example
    ...
    private AnswerValues parseWriteResponse(byte[] response) {
        if (response.length < 4 || response[0] != (byte) slaveAddress || (response[1] != 0x06 && response[1] != (byte)0x86)) {
            if(response[1] != (byte)0x06 && response[1] != (byte)0x86) {
                log.warn("IGM10: Invalid write response [1] != (0x06) && [1] != (0x86). Received: " + response[1]);
            }

            if(response[0] != (byte) slaveAddress){
                log.warn("IGM10: Invalid write response [0] != slaveAddress. SlaveID" + slaveAddress + " Received: " + response[0]);
            }

            if(response.length != 8){
                log.warn("IGM10: Invalid write response [length] != 8. Received: " + response.length);
            }
            log.warn("IGM10: Invalid write response");
            return null;
        }
        if (!validateCRC(response)) {
            log.warn("IGM10: CRC error in write response");
        }
        AnswerValues answerValues = new AnswerValues(1);
