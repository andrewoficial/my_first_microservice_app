...
public class Igm11Modbus implements SomeDevice, ProtocolComPort {
    private static final Logger log = Logger.getLogger(Igm11Modbus.class);
    @Getter
    ...
    private final Igm11ModbusCommandRegistry commandRegistry;
    ...
    private String devIdent = "IGM11";
    private byte[] rawCmd = null;
    private int slaveAddress = 1; // Default Modbus slave address

    public Igm11Modbus() {
        log.info("Created Igm11 protocol emulation object");
        this.comPort = null;
        this.commandRegistry = new Igm11ModbusCommandRegistry();
        this.commands = commandRegistry.getCommandList();
    }

    public Igm11Modbus(SerialPort port) {
        log.info("Created Igm11 protocol object");
        this.comPort = port;
        this.commandRegistry = new Igm11ModbusCommandRegistry();
        this.commands = commandRegistry.getCommandList();
    ...
    public List<byte[]> getBytesToSend() {
        ...
        if ("getConc".equals(cmdName)) {
            ...
            bytesList.add(commandRegistry.buildReadHolding(3, 1)); // Gas type, then perhaps another for range
        ...
        if (lastAnswerBytes != null && lastAnswerBytes.length > 0) {
            ...
            if (isKnown) {
                ...
                if (answerValues != null) {
                    ...
                    log.info("IGM11 Cant create answers obj (error in answer)");
                }
            } else {
                lastAnswer.setLength(0);
