...
import org.example.device.protErstevakMtp4d.ErstevakMtp4dCommandRegistry;
...
public class ERSTEVAK_MTP4D implements SomeDevice {
    ...
    private final ErstevakMtp4dCommandRegistry commandRegistry;
    ...
    public ERSTEVAK_MTP4D(){
        log.info("Создан объект протокола ERSTEVAK_MTP4D эмуляция");
        this.comPort = null;
        this.commandRegistry = new ErstevakMtp4dCommandRegistry();
        ...
        if(lastAnswerBytes != null && lastAnswerBytes.length > 0) {
            ...
            if (commands.isKnownCommand(cmdToSend)) { //Проверка наличия команды в реестре команд
                ...
                if(answerValues != null){
                    ...
                    log.info("ERSTEVAK_MTP4D Cant create answers obj (error in answer)");
                }
            }else {
                for (byte lastAnswerByte : lastAnswerBytes) {
                    lastAnswer.append((char) lastAnswerByte);
                }
                log.info("ERSTEVAK_MTP4D Cant create answers obj (unknown command)");
            }
        }else{
            log.info("ERSTEVAK_MTP4D empty received");
        }
    }
