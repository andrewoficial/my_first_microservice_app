...
public class BOTO800_DEVICE implements SomeDevice, ProtocolComPort {
    ...
    private final Boto800CommandRegistry commandRegistry;
    ...
    public BOTO800_DEVICE() {
        log.info("Создан объект протокола BOTO800_DEVICE эмуляция");
        this.comPort = null;
        this.commandRegistry = new Boto800CommandRegistry();
        this.commands = commandRegistry.getCommandList();
    }

    public BOTO800_DEVICE(SerialPort port) {
        log.info("Создан объект протокола BOTO800_DEVICE");
        this.comPort = port;
        this.commandRegistry = new Boto800CommandRegistry();
        this.commands = commandRegistry.getCommandList();
        ...
        if (lastAnswerBytes != null && lastAnswerBytes.length > 0) {
            ...
            if (cmd != null) {
                ...
                if (answerValues != null) {
                    ...
                    log.warn("BOTO800: parse failed for '{}'", cmdToSend);
                }
            } else {
                for (byte b : lastAnswerBytes) lastAnswer.append((char) b);
                log.warn("BOTO800: unknown command '{}'", cmdToSend);
            }
        }
    }
