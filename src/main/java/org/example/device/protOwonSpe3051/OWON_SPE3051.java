...
public class OWON_SPE3051 implements SomeDevice {
    private static final Logger log = Logger.getLogger(OWON_SPE3051.class);
    @Getter
    ...
    private String devIdent = "OWON_SPE3051";

    public OWON_SPE3051(){
        log.info("Создан объект протокола OWON_SPE3051 эмуляция");
        this.comPort = null;
        ...
        if(lastAnswerBytes != null && lastAnswerBytes.length > 0) {
            ...
            if (commands.isKnownCommand(cmdToSend)) { //Проверка наличия команды в реестре команд
                ...
                if(answerValues != null){
                    ...
                    log.info("OWON_SPE3051 Cant create answers obj (error in answer)");
                }
            }else {
                for (byte lastAnswerByte : lastAnswerBytes) {
                    lastAnswer.append((char) lastAnswerByte);
                }
                log.info("OWON_SPE3051 Cant create answers obj (unknown command)");
            }
        }else{
            log.info("OWON_SPE3051 empty received");
        }
    }
